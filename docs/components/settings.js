// Settings Component for MeauxOS
  ...
  return `
    ...
    <div style="padding: 0; max-width: 1600px; margin: 0 auto;">
      <div class="card" style="padding: 24px; text-align: center;">
        ...
        <p style="font-size: 13px; color: var(--text-muted);">API Endpoint: <code>/api/meauxaccess/settings</code></p>
      </div>
    </div>
  `;
}
...
export function init() {
  async function loadData() {
    try {
      const response = await fetch('/api/meauxaccess/settings');
      if (response.ok) {
